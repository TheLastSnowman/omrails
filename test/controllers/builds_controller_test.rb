...
class BuildsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post builds_url, params: { build: { comments: @build.comments, name: @build.name, url: @build.url, user_id: @build.user_id } }
    end
  ...
  test "should update build" do
    patch build_url(@build), params: { build: { comments: @build.comments, name: @build.name, url: @build.url, user_id: @build.user_id } }
